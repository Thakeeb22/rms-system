  ...
  try {
    ...
    if (results.length === 0) {
      return res.status(404).json({
        success: false,
        message:
          "No published results found for this student in the selected session and term.",
      });
    }
    ...
    if (term.termName === "Third Term") {
      if (average >= 70) {
        ...
        principalRemark =
          "Promoted to the next class. Satisfactory performance.";
      } else if (average >= 45) {
    ...
    const assessment = await ReportAssessment.findOne({
      ...
      if(!assessment){
        return res.status(404).json({
            success:false,
            message:"Student assessment has not been added."
        })
      }
